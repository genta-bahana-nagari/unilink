...
export default function EditResearchPage() {
  ...
  useEffect(() => {
    api.getResearchById(id).then((data) => {
      if (data) {
        setResearch(data);
        setForm({
          title: data.title,
          ...
          requiredParticipants: String(data.requiredParticipants),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: "Failed to update research study" });
    } finally {
  ...
  if (!research) {
    return (
      <div className="p-6 text-center">
        <p className="text-slate-500">Research study not found</p>
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Edit Research Study</h1>
          <p className="text-slate-500 mt-1">{research.title}</p>
        </div>
      </div>
